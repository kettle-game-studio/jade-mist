...
float voronoi2(float2 coord, float density, float shift = 1)
{
    ...
    for (int y = 0; y <= 1; ++y)
    {
        for (int x = 0; x <= 1; ++x)
        {
            float2 cell_center = (hash22(cell + float2(x, y)) - 0.5) * shift;
...
float voronoi3(float3 coord, float density, float shift = 1)
{
    ...
    for (int y = 0; y <= 1; ++y)
    {
        for (int x = 0; x <= 1; ++x)
        {
            for (int z = 0; z <= 1; ++z)
            {
                float3 cell_center = (hash33(cell + float3(x, y, z)) - 0.5) * shift;
...
float2 _normal_to_simplex_dim2(float2 coord)
{
    float dimension = 2;
...
float2 _simplex_to_normal_dim2(float2 coord)
{
    float dimension = 2;
...
{
    coord = _normal_to_simplex_dim2(coord);
    float2 cell = floor(coord * density);
    float2 offset = frac(coord * density);
    float2 points[3] = {
        float2(0, 0),
    ...
    offset = _simplex_to_normal_dim2(offset);
    float result = 1;
    for (int i = 0; i < 3; ++i)
    {
        float2 p = points[i];
        p = _simplex_to_normal_dim2(p + (hash22(cell + p) - 0.5) * shift * 0.4); // TODO: 0.4??
        result = min(result, distance(p, offset) * 1.7); // TODO: 1.7??
    }
    return result;
}

float3 _normal_to_simplex_dim3(float3 coord)
{
    float dimension = 3;
    float f = (sqrt(dimension + 1) - 1) / dimension;
...
float3 _simplex_to_normal_dim3(float3 coord)
{
    float dimension = 3;
    float g = (1 - 1 / sqrt(dimension + 1)) / dimension;
    float coord_sum = coord.x + coord.y + coord.z;
...
{
    coord = _normal_to_simplex_dim3(coord);
    ...
    offset = _simplex_to_normal_dim3(offset);
    float result = 1;
    for (int i = 0; i < 4; ++i)
    {
        float3 p = points[i];
        p = _simplex_to_normal_dim3(p + (hash33(cell + p) - 0.5) * shift * 0.3); // TODO: 0.3??
